...
import type { DrawMyResultDTO, DrawPayoutStatus } from "@digital-heroes/shared";
            ...
            <div key={r.drawId} style={{ border: "1px solid var(--color-border)", borderRadius: 10, padding: 14 }}>
              ...
              {r.prizeAmountCents > 0 && (
                <p style={{ margin: "8px 0 0" }}>
                  ...
                  {r.payoutStatus && <span style={{ color: "var(--color-text-muted)" }}> · Payout {PAYOUT_LABEL[r.payoutStatus].toLowerCase()}</span>}
                </p>
              )}
            </div>
          ))}
